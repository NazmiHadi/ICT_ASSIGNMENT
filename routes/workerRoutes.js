...
// Blocks deletion if other workers currently report to this one
// (WORKERS.ManagerID) — those need to be reassigned to a different
// manager first. Otherwise deletes the FULL_TIME/PART_TIME pay record
// (whichever applies) before deleting the WORKERS row itself, since
// those are child records tied to WorkID.
  ...
  try {
    ...
    if (managesOthers.rows[0].CNT > 0) {
      return res.status(409).json({
        success: false,
        message: "Other workers report to this person. Reassign them to a different manager before deleting."
      });
    }
    ...
    await conn.execute(`DELETE FROM WORKERS WHERE WorkID = :workId`, { workId });
