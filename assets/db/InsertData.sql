...
CREATE TABLE PRODUCTS (
    ...
    -- Actual filename saved on disk by the upload (e.g. prod-1735987200-a1b2.jpg)
    -- NULL until a worker uploads a picture. Never guess this from ProdID --
    -- that's what caused the broken image links.
...
-- =====================================================================
-- IF YOU ALREADY HAVE THESE TABLES CREATED (with data in them),
...
--    table, or you'll get a primary-key collision on the next insert.
--    Check first, e.g.: SELECT MAX(ProdID) FROM PRODUCTS;
ALTER TABLE CUSTOMERS MODIFY CustID GENERATED ALWAYS AS IDENTITY (START WITH 5001);
ALTER TABLE WORKERS   MODIFY WorkID GENERATED ALWAYS AS IDENTITY (START WITH 9192);
ALTER TABLE VENDORS   MODIFY VendID GENERATED ALWAYS AS IDENTITY (START WITH 7001);
ALTER TABLE CONTAINERS MODIFY ContID GENERATED ALWAYS AS IDENTITY (START WITH 3001);
ALTER TABLE PRODUCTS  MODIFY ProdID GENERATED ALWAYS AS IDENTITY (START WITH 2025);
ALTER TABLE ORDERS    MODIFY OrderID GENERATED ALWAYS AS IDENTITY (START WITH 8001);
ALTER TABLE PURCHASE  MODIFY PurchID GENERATED ALWAYS AS IDENTITY (START WITH 6001);